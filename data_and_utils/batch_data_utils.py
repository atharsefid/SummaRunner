...
class DataGenerator(object):
	...
	def get_train_val_batch(self, split='train'):
		...
		if enc_inp_np.shape[0] != params.batch_size:
			enc_inp_np = np.concatenate([enc_inp_np, np.zeros([params.batch_size-enc_inp_np.shape[0],params.doc_size,enc_inp_np.shape[-1]])], axis=0)
			dec_inp_np = np.concatenate([dec_inp_np, np.zeros([params.batch_size-dec_inp_np.shape[0],params.doc_size])], axis=0)
